...
{
    public class Chart_Bar : MouseMoveElement<Rect, Dictionary<string, double>>
    {
        ...
        protected override IEnumerable<Rect> GetObjects()
        {
            if (Data.Count == 0)
            ...
            IEnumerable<Rect> DoGetObjects()
            {
                ...
                foreach (var item in Data)
                {
                    ...
                    yield return new Rect(new RectangleF((float)xScr - 15, Math.Min((float)yTop, (float)yBottom), 30, Math.Abs((float)yTop - (float)yBottom))) { Value = item.Key};
                }
            }
        }
    }
